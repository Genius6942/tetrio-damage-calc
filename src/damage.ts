...
export const garbageCalcV2 = (
  ...
  let garbage = 0;
  const { spin: rawSpin, lines, piece, combo, b2b, enemies, perfectClear } = data;
  const {
    spinBonuses,
    comboTable,
    b2bChaining,
  ...
  if (spin && spinBonuses === "handheld" && piece !== "T") {
    garbage /= 2;
  }
    ...
    if (b2bChaining) {
      const b2bGains =
        ...
        (Math.floor(1 + Math.log1p(b2b * garbageData.backtobackBonusLog)) +
          (b2b == 1
            ? 0
            : (1 + (Math.log1p(b2b * garbageData.backtobackBonusLog) % 1)) / 3));
      garbage += b2bGains;
      console.log(
        b2bGains,
        (1 + (Math.log1p(b2b * garbageData.backtobackBonusLog) % 1)) / 3
  ...
  if (combo > 0) {
    if (comboTable === "multiplier") {
      ...
      if (combo > 1) {
        garbage = Math.max(
          Math.log1p(garbageData.comboMinifier * combo * garbageData.comboMinifierLog),
          garbage
        );
      }
    } else {
  ...
  if (lines > 0 && garbageTargetBonus !== "none") {
    ...
    if (garbageTargetBonus === "normal") {
      ...
      garbageBonus = Math.floor(targetBonus * garbageMultiplier);
    }
  }

  let l = Math.floor(garbage * garbageMultiplier);
  if (garbageAttackCap) {
  ...
  return {
    garbage: l + (perfectClear ? 10 * garbageMultiplier : 0),
    bonus: garbageBonus * garbageMultiplier,
  };
};
